...
import config
from bot_test1 import exchange_valute, telegram_bot
from responses import response_twitter_tweets, response_twitter_last5tweets_of_the_user, response_bank
...
def callback_inline_wrap(random_digit):
        ...
        try:
            if call.message:
                if str(
                    ...
                    telegram_bot.send_photo(call.message.chat.id,
...
def handleText_wrap(data):
    @telegram_bot.message_handler(content_types=['text'])  # handle with text
    def handleText(message):
        ...
        if "tweet=" in message.text or "tweets=" in message.text:
            ...
            response = response_twitter_last5tweets_of_the_user(message.text.replace(" ", "").split("=")[1])
            if "data" in response:
                for dataItem in response["data"]:
                    telegram_bot.send_message(
                        message.chat.id,
            ...
            data = response_bank(config.bank_api_currency)
